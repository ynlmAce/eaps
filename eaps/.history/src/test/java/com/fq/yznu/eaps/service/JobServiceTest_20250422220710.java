...
class JobServiceTest {
    ...
    void whenFindByStatus_thenReturnJobList() {
        ...
        verify(jobDao).findByStatus("open", PageRequest.of(0, 10));
    }
    ...
    void whenUpdateJob_thenReturnUpdatedJob() {
        ...
        Job updatedJob = jobService.updateJob(testJob);
    ...
    void whenCloseJob_thenVerifyStatusChange() {
        when(jobDao.findById(testJob.getId())).thenReturn(java.util.Optional.of(testJob));
        when(jobDao.save(any(Job.class))).thenReturn(testJob);

        jobService.closeJob(testJob.getId());
    ...
    void whenSearchJobs_thenReturnMatchingJobs() {
        Page<Job> jobPage = new PageImpl<>(Arrays.asList(testJob));
        when(jobDao.findByJobTitleContainingAndLocationAndJobTypeAndSalaryMinGreaterThanEqual(
                "Java", "昆明市", "full_time", 7000, PageRequest.of(0, 10)))
                .thenReturn(jobPage);

        Page<Job> found = jobService.searchJobs("Java", "昆明市", "full_time", 7000, PageRequest.of(0, 10));

        assertNotNull(found);
        assertEquals(1, found.getTotalElements());
        assertEquals(testJob.getJobTitle(), found.getContent().get(0).getJobTitle());
        verify(jobDao).findByJobTitleContainingAndLocationAndJobTypeAndSalaryMinGreaterThanEqual(
                "Java", "昆明市", "full_time", 7000, PageRequest.of(0, 10));
    }
}