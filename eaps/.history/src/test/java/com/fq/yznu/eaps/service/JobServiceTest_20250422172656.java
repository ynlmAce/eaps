...
import com.fq.yznu.eaps.mapper.JobMapper;
import com.fq.yznu.eaps.service.impl.JobServiceImpl;
...
class JobServiceTest {
    ...
    void whenFindByStatus_thenReturnJobList() {
        ...
        verify(jobMapper).selectList(any());
    }

    @Test
    void whenUpdateJob_thenReturnUpdatedJob() {
        when(jobMapper.selectById(testJob.getId()))
                .thenReturn(testJob);
        when(jobMapper.updateById(any(Job.class)))
                .thenReturn(1);
